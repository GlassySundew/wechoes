	...
	private static function build() : Array<Field> {
		...
		for ( field in fields ) {
			switch ( field.name ) {
				case "new", "_new":
					return
						Context.fatalError( "The constructor must be generated by macro. Instead, declare an onTemplateApplied() function.", field.pos );
				case "applyTemplateTo", "applyTemplateToSelf":
					return
						Context.fatalError( '${field.name} is reserved. Instead, declare an onTemplateApplied() function.', field.pos );
				case "onTemplateApplied":
					switch field.kind {
						case FFun( f ):
							if (
								f.args.length == 1
								&& f.args[0].name == 'world'
								&& f.args[0].type != null
								&& f.args[0].type.toString() == "echoes.World"
							)
								onTemplateApplied = field;
						case e:
							Context.fatalError( 'field type $e not supported, should be a function ', field.pos );
					}
				default:
			}
		}
		...
		for ( field in fields.copy() ) {
			...
			var fieldNameCapitalized = field.name.charAt( 0 ).toUpperCase() + field.name.substr( 1 );

			final getter : String = "get" + fieldNameCapitalized;
			final setter : String = "set" + fieldNameCapitalized;

			fields.pushFields( macro class Accessors {
				public inline function $getter( world : echoes.World ) : Null<$componentType> {
					return this.get( world, ( _ : $componentType ) );
				}

				public inline function $setter( value : Null<$componentType>,
					world : echoes.World ) : Null<$componentType> {
					this.add( world, value );
					return value;
				}
			} );
		}
		...
		inline function addFunctions( definition : TypeDefinition ) : Void {
			for ( field in definition.fields ) {
				switch ( field.kind ) {
					case FFun( f ):
						// f.args = parameters.concat(f.args);
		...
		arguments.unshift( macro cast world );
		superArguments.unshift( macro cast world );
		addFunctions( macro class Constructor {
			public static inline function applyTemplateTo( entity : echoes.Entity, world : echoes.World ) : $templateType {
				( cast entity : $templateType ).applyTemplateToSelf( $a{arguments} );
				return cast entity;
			}

			public inline function new( world : echoes.World ) {
				this = cast new echoes.Entity( world );
		...
		for ( parameter in parameters ) {
			applyToSelfExprs.push( macro this.addIfMissing( world, $i{parameter.name} ) );
		}
		for ( component in knownComponents ) {
			applyToSelfExprs.push( macro this.addIfMissing( world, ${component.value} ) );
		}
		if ( parents.length > 1 ) {
			applyToSelfExprs.push( macro @:privateAccess this.applyTemplateToSelf( $a{superArguments} ) );
		}
		if ( onTemplateApplied != null ) {
			applyToSelfExprs.push( macro $i{onTemplateApplied.name}( world ) );
		}
		...
		addFunctions( macro class ApplyToSelf {
			@:noCompletion private function applyTemplateToSelf( world : echoes.World ) : Void$b{applyToSelfExprs}
		} );
		...
		for ( component in knownComponents ) {
			...
			removeFromSelfExprs.push( macro @:privateAccess this.remove( world, ( _ : $type ) ) );
		}
		if ( parents.length > 1 ) {
			removeFromSelfExprs.push( macro if ( recursive )
				@:privateAccess this.removeTemplateFromSelf( true, world ) );
		}

		fields = ( macro class RemoveTemplate {
			@:noCompletion
			public static inline function removeTemplateFrom(
				entity : echoes.Entity,
				world : echoes.World,
				?recursive : Bool = false
			) : echoes.Entity {
				( cast entity : $templateType ).removeTemplateFromSelf( ${parents.length > 1 ? macro recursive : macro false}, world );
				return cast entity;
			}

			@:noCompletion private function removeTemplateFromSelf( recursive : Bool, world : echoes.World ) : Void
				$b{removeFromSelfExprs}
		...
		fields[0].doc = ( knownComponents.length == 1 ? 'Removes `${new Printer().printComplexType( knownComponents[0].type )}`' : 'Removes `${type.name}`\'s ${knownComponents.length} components' )
