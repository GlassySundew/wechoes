	...
	private static function build():Array<Field> {
		...
		for(field in fields.copy()) {
			...
			fields.pushFields(macro class Accessors {
				...
				public inline function $setter(value:Null<$componentType>, world: echoes.World):Null<$componentType> {
		...
		if(parents.length > 1) {
			removeFromSelfExprs.push(macro if(recursive) @:privateAccess this.removeTemplateFromSelf(true, world));
		}
		...
		fields = (macro class RemoveTemplate {
			...
			@:noCompletion private function removeTemplateFromSelf(recursive:Bool, world: echoes.World):Void $b{ removeFromSelfExprs }
